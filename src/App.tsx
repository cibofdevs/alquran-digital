    ...
    const fetchSurahs = async () => {
      try {
        ...
        if (data.code === 200 && Array.isArray(data.data)) {
          ...
          // Load last read surah after surahs are fetched
          const lastRead = localStorage.getItem(LAST_READ_KEY);
          if (lastRead) {
            const { surah, ayah } = JSON.parse(lastRead) as LastRead;
            setSelectedSurah(surah);
            setPendingScroll({ surah, numberInSurah: ayah });
          }
        }
      } catch (error) {
      ...
      try {
        ...
        if (arabicData.code === 200 && englishData.code === 200) {
          ...
          const lastRead: LastRead = {
            surah: number,
            ayah: pendingScroll?.numberInSurah || 1,
            timestamp: Date.now()
          };
